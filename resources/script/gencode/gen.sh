# 生成api业务代码 ， 进入"服务/cmd/material/desc"目录下，执行下面命令
# goctl material go -material *.material -dir ../  --style=goZero
...
#
# 1）goctl >= 1.3 进入"服务/cmd/auth/pb"目录下，执行下面命令
#    goctl auth protoc *.proto --go_out=../ --go-grpc_out=../  --zrpc_out=../ --style=goZero
#    去除proto中的json的omitempty
#    mac: sed -i "" 's/,omitempty//g' *.pb.go
#    linux: sed -i 's/,omitempty//g' *.pb.go
# 2）goctl < 1.3 进入"服务/cmd"目录下，执行下面命令
#    goctl auth proto -src auth/pb/*.proto -dir ./auth --style=goZero
#    去除proto中的json的omitempty
#    mac: sed -i "" 's/,omitempty//g'  ./auth/pb/*.pb.go
#    linux: sed -i 's/,omitempty//g'  ./auth/pb/*.pb.go
